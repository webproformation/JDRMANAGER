      ...
      <div className="bg-[#151725] border border-white/5 rounded-[2.5rem] p-6 shadow-2xl">
        <div className="flex justify-between items-end border-b border-white/5 pb-4 mb-6">
          <h4 className="text-sm font-black uppercase text-white flex items-center gap-3 tracking-widest">
            <Backpack size={18} className="text-cyan-400"/> 
            Gestion de l'Inventaire
          </h4>
          
          {/* JAUGE DE CHARGE DYNAMIQUE */}
          <div className={`text-[10px] font-black uppercase flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all ${
            isOverloaded 
              ? 'bg-red-500/20 text-red-400 border-red-500/30 animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.3)]' 
              ...
              return (
                <div key={loc.id} className={`bg-[#0f111a] border ${loc.border} rounded-[2rem] p-5 shadow-inner transition-all hover:border-opacity-50`}>
                  ...
                  <div className="space-y-3">
                    {groupItems.map(item => {
                      // Détection si l'objet va affecter le combat (Armes/Armures Équipées)
                      const typeLower = (item.type || '').toLowerCase();
                      const isCombatActive = item.location === 'equipped' && (
                        typeLower.includes('weapon') || typeLower.includes('arme') || 
                        typeLower.includes('armor') || typeLower.includes('armure') || 
                        typeLower.includes('shield') || typeLower.includes('bouclier')
                      );
                        ...
                        <div key={item.originalIndex} className="bg-[#1a1d2d] border border-white/5 rounded-2xl p-3 flex items-center justify-between group hover:border-white/20 transition-all shadow-lg">
                          ...
                          <div className="flex-1 min-w-0 pr-4">
                            <div className="font-bold text-white text-sm truncate flex items-center gap-2">
                              {item.name}
                              {isCombatActive && (
                                <span className="bg-teal-500/20 text-teal-400 px-2 py-0.5 rounded-full text-[8px] tracking-widest uppercase">
                                  Actif en Combat
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              ...
                              <input 
                                type="text" 
                                placeholder="Détail (ex: Main, Botte...)"
                                value={item.subLocation || ''}
                                onChange={(e) => updateField(item.originalIndex, 'subLocation', e.target.value)}
                          ...
                          <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 bg-black/40 p-1 rounded-xl border border-white/5">
                              <button 
                                type="button" 
                                onClick={() => updateField(item.originalIndex, 'quantity', item.quantity - 1)} 
                                className="w-7 h-7 flex items-center justify-center bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition-all font-black text-lg"
                              >
                                -
                              </button>
                              ...
                              <button 
                                type="button" 
                                onClick={() => updateField(item.originalIndex, 'quantity', item.quantity + 1)} 
                                className="w-7 h-7 flex items-center justify-center bg-teal-500/10 text-teal-400 rounded-lg hover:bg-teal-500/20 transition-all font-black text-lg"
                              >
                                +
                              </button>
                            </div>
                            ...
                            <button 
                              ...
                              className="p-2.5 text-silver/20 hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-all shadow-inner" 
                              title="Jeter l'objet"
                            >
                              <Trash2 size={18} />
                            </button>
              ...
              <div key={item.id} className="bg-[#151725] border border-white/5 rounded-2xl overflow-hidden group hover:border-cyan-500/30 transition-all">
                ...
                {isExpanded && (
                  <div className="bg-black/40 p-5 border-t border-white/5 text-xs text-silver space-y-3">
                    <p className="leading-relaxed border-l-2 border-cyan-500/50 pl-3 italic">
                      {item.description || 'Aucune description disponible dans les archives.'}
                    </p>
                  </div>
                )}
              </div>
