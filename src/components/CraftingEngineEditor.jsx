...
export default function CraftingEngineEditor({ value = {}, onChange }) {
  // SÉCURITÉ : On s'assure que craftingData et ses tableaux existent toujours
  const craftingData = value.crafting || {};
  const ingredients = Array.isArray(craftingData.ingredients) ? craftingData.ingredients : [];
  const steps = Array.isArray(craftingData.steps) ? craftingData.steps : [];

  const [dbItems, setDbItems] = useState({ 
    minerals: [], plants: [], materials: [], spells: [], animals: [], monsters: [], items: [] 
  });
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [minRes, plantRes, matRes, spellRes, animRes, monRes, itemRes] = await Promise.all([
          ...
          supabase.from('monsters').select('id, name').order('name'),
          supabase.from('items').select('id, name').order('name')
        ]);

        setDbItems({
          minerals: minRes.data || [],
          plants: plantRes.data || [],
          materials: matRes.data || [],
          spells: spellRes.data || [],
          animals: animRes.data || [],
          monsters: monRes.data || [],
          items: itemRes.data || []
        });
      } catch (error) {
  ...
  const updateCrafting = (updates) => {
    onChange({ 
      ...
      crafting: { 
        ...craftingData, 
        ingredients, // Maintient le tableau si non modifié
        steps,       // Maintient le tableau si non modifié
        ...updates 
      } 
    });
  };
  ...
  const addIngredient = () => {
    const newIngredients = [...ingredients, { type: 'mineral', id: '', name: '', quantity: 1 }];
  ...
  const addStep = () => {
    const newSteps = [...steps, { description: '', duration: '', dc_modifier: '' }];
  ...
  if (loading) return <div className="p-8 text-center text-silver/50 italic text-sm animate-pulse">Chargement de l'encyclopédie des matériaux...</div>;
    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-[#0f111a] border border-white/5 rounded-[2rem] p-6 shadow-inner">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/5">
              ...
              <button type="button" onClick={addIngredient} className="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 p-2 rounded-xl transition-all border border-amber-500/20 flex items-center gap-2 text-[10px] font-black uppercase">
                <Plus size={14} /> Ajouter
              </button>
            </div>

            {ingredients.length === 0 ? (
              <div className="text-center py-10 border-2 border-dashed border-white/5 rounded-2xl text-silver/30 text-xs italic">Aucun ingrédient défini.</div>
            ) : (
                ...
                {ingredients.map((ing, idx) => (
                  <div key={idx} className="bg-[#151725] border border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center group hover:border-amber-500/30 transition-all">
                    <div className="flex-1 w-full flex items-center gap-3">
                      <select value={ing.type} onChange={(e) => updateIngredient(idx, 'type', e.target.value)} className="bg-black/40 text-silver text-[10px] font-black uppercase tracking-wider border border-white/10 rounded-xl px-3 py-2.5 outline-none focus:border-amber-500">
                        <option value="mineral">Minéral</option>
                        <option value="plant">Plante</option>
                        <option value="material">Matériau</option>
                        <option value="animal">Animal</option>
                        <option value="monster">Monstre</option>
                        <option value="item">Objet/Arme</option>
                        <option value="spell">Sortilège</option>
                      </select>

                      <select value={ing.id} onChange={(e) => updateIngredient(idx, 'id', e.target.value)} className="flex-1 bg-black/40 text-white text-xs font-bold border border-white/10 rounded-xl px-3 py-2.5 outline-none focus:border-amber-500">
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                      ...
                      <button type="button" onClick={() => removeIngredient(idx)} className="p-2.5 text-silver/30 hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-all"><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-[#0f111a] border border-white/5 rounded-[2rem] p-6 shadow-inner">
               ...
               <input type="number" value={craftingData.base_dc || 10} onChange={(e) => updateCrafting({ base_dc: parseInt(e.target.value) || 10 })} className="w-full bg-[#151725] text-xl font-black text-center text-white border border-white/10 rounded-xl p-2 outline-none focus:border-amber-500/50 transition-all [&::-webkit-inner-spin-button]:appearance-none" />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-[#0f111a] border border-white/5 rounded-[2rem] p-6 shadow-inner">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/5">
              <h4 className="text-xs font-black text-teal-400 uppercase tracking-widest flex items-center gap-2"><Scroll size={16} /> Étapes de Fab.</h4>
              <button type="button" onClick={addStep} className="bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 p-2 rounded-xl transition-all border border-teal-500/20 flex items-center gap-2 text-[10px] font-black uppercase"><Plus size={14} /> Ajouter</button>
            </div>
            {steps.length === 0 ? (
              <div className="text-center py-8 border-2 border-dashed border-white/5 rounded-2xl text-silver/30 text-xs italic">Aucune étape définie.</div>
            ) : (
              <div className="space-y-4">
                {steps.map((step, idx) => (
                  <div key={idx} className="relative bg-[#1a1d2d] border border-white/5 rounded-2xl p-4 pl-10 group">
                    <div className="absolute left-3 top-4 w-5 h-5 bg-teal-500/20 text-teal-400 rounded-full flex items-center justify-center text-[10px] font-black border border-teal-500/30">{idx + 1}</div>
                    <button type="button" onClick={() => removeStep(idx)} className="absolute right-3 top-3 p-1.5 text-silver/20 hover:text-red-400 bg-black/40 rounded-lg opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={12} /></button>
                    <div className="space-y-3 mt-1">
                      ...
                      <textarea value={step.description || ''} onChange={(e) => updateStep(idx, 'description', e.target.value)} placeholder="Décrivez l'action requise..." rows="2" className="w-full bg-[#151725] text-xs text-silver/70 border border-white/10 rounded-xl p-3 outline-none focus:border-teal-500/50 resize-none" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-red-900/10 border border-red-500/20 rounded-[2rem] p-5 relative overflow-hidden">
              ...
              <textarea value={craftingData.critical_failure || ''} onChange={(e) => updateCrafting({ critical_failure: e.target.value })} placeholder="Conséquence dramatique..." rows="2" className="w-full bg-black/40 text-xs text-white border border-red-500/10 rounded-xl p-3 outline-none focus:border-red-500/50 relative z-10 resize-none" />
            </div>
          </div>
        </div>
      </div>
