...
export default function MediaLibrary({ onSelect, worldIdFilter = null }) {
    ...
    try {
      for (let i = 0; i < selectedFiles.length; i++) {
        ...
        await supabase.from('media_items').insert([{
          name: file.name.split('.')[0], url: publicUrl,
          folder_id: currentFolderId, world_id: selectedWorld === 'all' ? null : selectedWorld
        }]);
      }
      fetchFiles();
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[520px] bg-[#16192a]/60 backdrop-blur-xl border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl transition-all relative">
        ...
        <div className="p-2.5 border-b border-white/5 bg-black/10 flex flex-wrap gap-2 justify-between items-center">
          <div className="flex items-center gap-2 flex-1">
            <div className="relative flex-1 max-w-xs">
              ...
              <input type="text" placeholder="Filtrer..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-black/20 border border-white/5 rounded-md pl-7 pr-3 py-1.5 text-[10px] focus:ring-1 focus:ring-[#2DD4BF]/50 outline-none h-7" />
            </div>
            <div className="w-28">
          ...
          <label className={`flex items-center gap-1.5 px-3 py-1 rounded-md cursor-pointer font-black uppercase text-[8px] h-7 ${uploading ? 'bg-gray-600' : 'bg-[#2DD4BF]/10 text-[#2DD4BF] border border-[#2DD4BF]/20 hover:bg-[#2DD4BF]/20'}`}>
      ...
      {isEditing && selectedItem && (
        <div className="w-full lg:w-64 bg-[#0c0e18]/90 backdrop-blur-3xl border-l border-white/10 flex flex-col min-h-0 animate-in slide-in-from-right duration-300 z-10 shadow-[-20px_0_50px_rgba(0,0,0,0.5)]">
          <div className="p-2.5 border-b border-white/5 flex justify-between items-center bg-black/20">
            ...
            <button onClick={() => setIsEditing(false)} className="text-white/20 hover:text-white"><X size={14} /></button>
          </div>
          <div className="flex-1 overflow-y-auto p-3.5 space-y-4" style={noScrollbarStyle}>
            <div className="aspect-video rounded-md overflow-hidden border border-white/5 bg-black/40 shadow-inner"><img src={selectedItem.url} className="w-full h-full object-contain" /></div>
            <div className="space-y-3">
              <div>
                <label className="text-[7px] font-black text-[#2DD4BF]/50 uppercase tracking-widest mb-1 block">Nom</label>
                <input type="text" value={selectedItem.name || ''} onChange={(e) => setSelectedItem({...selectedItem, name: e.target.value})} className="w-full bg-black/20 border border-white/5 rounded-md px-2 py-1 text-[10px] text-white outline-none focus:border-teal-500/50" />
              </div>

              <div>
                <label className="text-[7px] font-black text-[#2DD4BF]/50 uppercase tracking-widest mb-1 block">Légende / Note</label>
                <textarea rows={2} value={selectedItem.legend || ''} onChange={(e) => setSelectedItem({...selectedItem, legend: e.target.value})} className="w-full bg-black/20 border border-white/5 rounded-md px-2 py-1 text-[10px] text-white outline-none focus:border-teal-500/50 resize-none" placeholder="..." />
              </div>

              <div>
                <label className="text-[7px] font-black text-[#2DD4BF]/50 uppercase tracking-widest mb-1 block">Lien Monde</label>
                <VTTSelect value={selectedWorld} options={[{value: 'all', label: 'Mondes'}, ...worlds.map(w => ({value: w.id, label: w.name}))]} onChange={(val) => setSelectedItem({...selectedItem, world_id: val})} />
              </div>
              <div>
                <label className="text-[7px] font-black text-[#2DD4BF]/50 uppercase tracking-widest mb-1 block">Catégorie</label>
                <VTTSelect value={selectedItem.folder_id} options={[{value: '', label: 'Global'}, ...getFolderOptions(null)]} onChange={(val) => setSelectedItem({...selectedItem, folder_id: val})} />
              </div>
            </div>
          </div>
          <div className="p-2.5 border-t border-white/5 bg-black/20 flex flex-col gap-2">
            {onSelect && (
               <button 
                ...
                className="w-full flex items-center justify-center gap-1.5 bg-teal-500 text-black rounded-md font-black uppercase text-[8px] h-8 hover:bg-teal-400 transition-all shadow-lg shadow-teal-500/10"
              >
                <CheckCircle2 size={12} /> Sélectionner cette image
              </button>
            )}
            ...
            <div className="flex gap-2">
              <button onClick={() => {
                setDialog({
                  isOpen: true, type: 'confirm', title: 'Supprimer image', message: 'Effacer définitivement cet actif ?',
                  onConfirm: async () => {
                    await supabase.from('media_items').delete().eq('id', selectedItem.id);
                    setSelectedItem(null); setIsEditing(false); fetchFiles();
                  }
                });
              }} className="p-1.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-md hover:bg-red-500/20 transition-all"><Trash2 size={14} /></button>
              
              <button onClick={saveMediaInfo} className="flex-1 flex items-center justify-center gap-1.5 bg-white/5 text-white/40 border border-white/10 rounded-md font-black uppercase text-[8px] h-8 hover:bg-white/10 hover:text-white transition-all"><Save size={11} /> Mettre à jour les infos</button>
            </div>
          </div>
        </div>
      )}
