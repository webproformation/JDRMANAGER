...
export default function EnhancedEntityForm({
  ...
  // --- BLOQUAGE STRICT DU SCROLL EXTERNE ---
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-4 md:p-8 overflow-hidden">
      ...
      {/* CONTENEUR IDENTIQUE À DETAILHEADER */}
      <div className="relative w-full h-full max-w-7xl bg-[#0f111a] sm:rounded-[3rem] border border-white/5 shadow-[0_0_100px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-500">
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          {/* NAVIGATION VERTICALE EXACTEMENT COMME DETAILHEADER */}
          ...
          <div ref={contentRef} className="flex-1 overflow-y-auto p-10 lg:p-16 space-y-12 no-scrollbar scroll-smooth">
             <form id="entity-form" onSubmit={handleSubmit} className="max-w-5xl mx-auto pb-24">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#0f111a] via-[#0f111a] to-transparent pointer-events-none flex justify-end z-20">
             <div className="pointer-events-auto flex gap-6 items-center">
               ...
               <button 
                 ...
                 form="entity-form" 
                 disabled={loading}
                 className={`flex items-center gap-3 px-10 py-4 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl transition-all ${
                   loading ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-500 hover:to-cyan-500 text-white hover:scale-105 active:scale-95 shadow-teal-500/20'
                 }`}
               >
