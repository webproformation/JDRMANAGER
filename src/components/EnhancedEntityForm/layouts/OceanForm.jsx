    ...
    if (name === 'image_url') {
      return (
        <div key={name} className="space-y-3 h-full">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 ml-1">Visuel des Eaux</label>
          <div 
            ...
            className={`group relative h-[calc(100%-24px)] min-h-[300px] rounded-[2.5rem] overflow-hidden border-2 border-dashed transition-all cursor-pointer ${
              formData.image_url ? 'border-transparent shadow-2xl' : 'border-white/10 bg-black/20 hover:border-cyan-500/30'
            }`}
          >
            {formData.image_url ? (
              <>
                ...
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-all">
                  <div className="p-4 bg-cyan-500/20 rounded-2xl border border-cyan-500/40 text-cyan-400"><ImageIcon size={32} /></div>
                  <span className="mt-4 text-[10px] font-black uppercase tracking-[0.2em] text-white">Changer l'image</span>
                </div>
              </>
            ) : (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white/10 group-hover:text-cyan-500/40 transition-colors">
                <Upload size={40} className="mb-3" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">Sélectionner une vue marine</span>
              </div>
            )}
          </div>
        </div>
    ...
    <div className="relative">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pr-2">
        ...
        {activeTab === 'hazards_tab' && (
          <div className="w-full pt-4">{renderFieldByName('hazards', 'w-full')}</div>
        )}
