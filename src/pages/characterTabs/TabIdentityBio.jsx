...
const generateMorphology = (sizeCat) => {
  ...
  switch (sizeCat) {
    case 'small':
      height = Math.floor(Math.random() * (130 - 80 + 1)) + 80; // 80cm à 130cm
      bmi = Math.random() * (30 - 20) + 20; // Corpulence dense (façon Nain/Gnome)
      weight = Math.round(bmi * Math.pow(height / 100, 2));
      speed = 7.5;
      break;
    case 'large':
      height = Math.floor(Math.random() * (300 - 220 + 1)) + 220; // 220cm à 300cm
      bmi = Math.random() * (35 - 25) + 25; // Ossature très massive
      weight = Math.round(bmi * Math.pow(height / 100, 2));
      speed = 12;
      break;
    case 'medium':
    default:
      height = Math.floor(Math.random() * (200 - 150 + 1)) + 150; // 150cm à 200cm
      bmi = Math.random() * (28 - 18) + 18; // De svelte à robuste (Humain standard)
      weight = Math.round(bmi * Math.pow(height / 100, 2));
      speed = 9;
      break;
  }
