...
import { 
  ...
  Zap, Eye, Activity, Crosshair, ScrollText, Coins, Package, 
  Weight, ChevronRight, Skull 
} from 'lucide-react';
        ...
        {stats.map(s => {
          ...
          return (
            <div key={s.k} className="bg-black/20 p-4 rounded-[1.5rem] border border-white/5 text-center shadow-inner flex flex-col items-center group hover:border-[#2DD4BF]/30 transition-all">
               ...
               <div className="flex items-center justify-center w-full bg-black/40 rounded-xl overflow-hidden border border-white/10 h-[40px]">
                  <button type="button" onClick={(e) => { e.preventDefault(); handleAttrChange(s.k, Math.max(1, score - 1)); }} className="px-3 h-full hover:bg-white/5 text-silver font-black transition-colors">-</button>
                  ...
                  <button type="button" onClick={(e) => { e.preventDefault(); handleAttrChange(s.k, score + 1); }} className="px-3 h-full hover:bg-white/5 text-silver font-black transition-colors">+</button>
               </div>
      ...
      render: (_, item) => {
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-black/20 p-5 rounded-2xl border border-[#2DD4BF]/20 text-center flex flex-col justify-center items-center shadow-inner">
          ...
          {(item.data?.arsenal || []).map((w, i) => (
            <div key={i} className="flex justify-between items-center bg-black/20 p-5 rounded-2xl border border-white/5 hover:border-amber-500/30 transition-all group shadow-lg">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-amber-500/5 rounded-xl border border-amber-500/10 group-hover:bg-amber-500/10 transition-colors">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                {[1,2,3,4,5,6,7,8,9].map(lvl => {
                   ...
                   return (
                     <div key={lvl} className="p-3 rounded-2xl bg-purple-900/20 border border-purple-500/30 flex flex-col items-center shadow-inner">
              ...
              <div className="space-y-4">
                {levels.map(lvl => (
                  <div key={lvl} className="bg-black/20 p-5 rounded-2xl border border-white/5 hover:border-purple-500/30 transition-all">
...
export const craftingTab = {
  id: 'crafting',
  label: 'Établi & Alchimie',
  icon: Hammer,
  fields: [
    { 
      name: 'crafting_bench', 
      isVirtual: true,
      label: 'Forge Tactique', 
      type: 'custom', 
      render: () => <div className="text-center text-silver/40 text-[10px] font-black uppercase tracking-widest py-16 bg-black/20 rounded-[3rem] border border-dashed border-white/5 animate-pulse">Interaction artisanale disponible en mode Édition uniquement.</div>,
      component: ({ formData, onFullChange }) => (
      ...
      render: (_, item) => (
        <div className="grid grid-cols-5 gap-2 mb-6">
          {['pc', 'pa', 'pe', 'po', 'pp'].map(coin => {
             ...
             return (
               <div key={coin} className="bg-black/40 p-3 rounded-xl border border-white/5 text-center shadow-inner group hover:scale-105 transition-transform">
                 <span className="text-[8px] uppercase font-black text-silver/40 block mb-1">{labels[coin]}</span>
